...
Если наш сервис не устраивает, то можно развернуть свою копию, внеся изменения в настройки или даже код.
...
Работает аналогично предыдущему способу. Эту ссылку также можно вставлять в `<img>`. Но здесь больше возможностей настроек.
...
Возвращает JSON-объект, который содержит URL загруженного изображения, а также информацию о том, сколько еще изображений можно загрузить в ближайшее время (ratelimit).
...
* `detailed` - Если 1, то отображается также дополнительная информация по загруженным изображениям, например дата и время их загрузки.

Все опции необязательны. `limit` по умолчанию 10.

#### `GET /who/<upload_key>` - кто загружал данное изображение.
...
* `from`, `limit` - пагинация.
* `detailed` - Если 1, то отображается также дополнительная информация по загруженным изображениям, например дата и время их загрузки каждым аккаунтом.
